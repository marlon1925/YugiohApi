...
passport.use(
  new LocalStrategy(
    ...
    async (email, password, done) => {
      ...
      if (!userBDD)
        return done("Lo sentimos, el email no se encuentra registrado", false);
      // VALIDACION DE LAS CONTRASEÑAS
      const passwordUser = await userBDD.matchPassword(password);
      // VALIDACION DEL PASSWORD DEL FORMULARIO VS EL DE LA BASE DE DATOS
      if (!passwordUser)
        return done("Lo sentimos, los passwords no coinciden", false);
      if (userBDD.confirmEmail === false)
        return done(
          "Lo sentimos, debe verificar la cuenta en su correo electrónico",
          false
        );
